...
# 浏览 apk
def BrowserApk():
    path = QtWidgets.QFileDialog.getOpenFileName(mainwindow, "选择APK", json.loads(readtxt(homePath + "/.config/uengine-runner/FindApk.json"))["path"], "APK 文件(*.apk);;所有文件(*.*)")
    if path[0] == "":
        return
    apkPath.setCurrentText(path[0])
    try:
        WriteTxt(homePath + "/.config/uengine-runner/FindApk.json", json.dumps({"path": os.path.dirname(path)}))  # 写入配置文件
    except:
        pass
...
infoLayout.addWidget(magiskDeltoInstallStatus, 1, 1)
...
waydroidClean.triggered.connect(lambda: threading.Thread(target=os.system, args=[f"pkexec rm -rfv  '~/.local/share/waydroid/data/*' ; zenity --info --text=删除完成！ --no-wrap"]).start())
