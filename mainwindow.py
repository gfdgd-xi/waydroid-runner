...
""")
        ...
        button0 = chooseWindow.addButton(choices[0], QtWidgets.QMessageBox.ButtonRole.ActionRole)
        button1 = chooseWindow.addButton(choices[1], QtWidgets.QMessageBox.ButtonRole.ActionRole)
        button2 = chooseWindow.addButton(choices[2], QtWidgets.QMessageBox.ButtonRole.ActionRole)
        button3 = chooseWindow.addButton(choices[3], QtWidgets.QMessageBox.ButtonRole.ActionRole)
        button4 = chooseWindow.addButton(choices[4], QtWidgets.QMessageBox.ButtonRole.ActionRole)
        button5 = chooseWindow.addButton(choices[5], QtWidgets.QMessageBox.ButtonRole.ActionRole)
        button6 = chooseWindow.addButton(choices[6], QtWidgets.QMessageBox.ButtonRole.ActionRole)
        button7 = chooseWindow.addButton(choices[7], QtWidgets.QMessageBox.ButtonRole.ActionRole)
...
if os.system("which waydroid"):
    if QtWidgets.QMessageBox.question(None, "提示", "您还未安装 Waydroid，是否立即安装？") == QtWidgets.QMessageBox.StandardButton.Yes:
...
# 程序栏
#settingProgramAction = QtGui.QAction("设置程序")
checkWayland = QtGui.QAction("检测是否是在 Wayland 下运行该程序")
installSway = QtGui.QAction("安装 Sway")
installWeston = QtGui.QAction("安装 Weston")
exitProgramAction = QtGui.QAction("退出程序")
...
# Waydroid 栏
waydroidAutoSetting = QtGui.QAction("一键安装/设置 Waydroid")
waydroidAutoSetting.setDisabled(True)
installWaydroidCNAction = QtGui.QAction("安装 Waydroid 本体（国内源）")
installWaydroidAction = QtGui.QAction("安装 Waydroid 本体（官方源）")
waydroidStatus = QtGui.QAction("Waydroid 状态")
waydroidLog = QtGui.QAction("查看 Waydroid 日志")
restartWaydroidContainer = QtGui.QAction("重启 Waydroid 服务进程")
iconManager = QtGui.QAction("快捷方式管理工具")
waydroidRemoveAllDesktop = QtGui.QAction("移除所有 Waydroid 快捷方式")
waydroidShowFullUI = QtGui.QAction("显示 Waydroid Android 完整界面")
waydroidGetAndroidId = QtGui.QAction("获取Waydroid的安卓id")
waydroidChangeGPU = QtGui.QAction("切换 GPU")
if os.getenv("XDG_SESSION_TYPE") == "x11":
    waydroidRunInX11Setting = QtGui.QAction("X11 运行 Waydroid 设置")
    waydroidRunInX11SettingWindow = X11Chooser()
else:
    waydroidRunInX11Setting = QtGui.QAction("X11 运行 Waydroid 设置（Wayland 无需设置）")
...
waydroidSessionStart = QtGui.QAction("开启")
waydroidSessionStop = QtGui.QAction("关闭")
...
waydroidMenu.addSeparator()
waydroidMenu.addAction(waydroidGetAndroidId)
...
# 容器配置栏
downloadImageCN = QtGui.QAction("下载 Waydroid 容器镜像")
magiskInstall = QtGui.QAction("安装或更新 Magisk")
libhoudiniInstall = QtGui.QAction("安装或更新 Libhoudini 翻译器")
widevineInstall = QtGui.QAction("安装或更新 Widevine")
waydroidLaguage = QtGui.QAction("设置 Waydroid 容器语言为中文")
waydroidClipboard = QtGui.QAction("开启 Waydroid 剪切板互通功能")
#multiWindowsSet = QtGui.QAction("开启 Waydroid 多窗口")
#doNotRotate = QtGui.QAction("")
waydroidAppListShow = QtGui.QAction("显示 Waydroid 安装的所有应用")
waydroidUpgradImage = QtGui.QAction("升级 Waydroid 容器（只限使用官方镜像安装方法）")
waydroidShell = QtGui.QAction("进入 Waydroid Shell")
...
configMenu.addAction(waydroidClipboard)
configMenu.addSeparator()
#configMenu.addAction(multiWindowsSet)
multiWindowsSet = configMenu.addMenu("设置 Waydroid 多窗口")
multiWindowsSetTips0 = QtGui.QAction("注：启用该功能后可能出现无法正常显示主界面的问题")
multiWindowsSetTips1 = QtGui.QAction("目前只支持部分桌面环境，不支持 Sway/Weston")
multiWindowsSetOn = QtGui.QAction("开启多窗口")
multiWindowsSetOff = QtGui.QAction("关闭多窗口")
...
doNotRotateTips = QtGui.QAction("此功能默认开启")
doNotRotateOn = QtGui.QAction("开启")
doNotRotateOff = QtGui.QAction("关闭")
...
waydroidClipboard.triggered.connect(lambda: threading.Thread(target=RunBash, args=[f"python3 '{programPath}/Runner_tools/SystemConfigs/Clipboard-enable.py'"]).start()) #剪切板添加(试验性功能!)
waydroidRepairUnshowMouse = configMenu.addMenu("修复 Waydroid 多窗口在部分桌面环境无法显示鼠标的问题")
waydroidRepairUnshowMouseTips = QtGui.QAction("此功能默认关闭（如果能正常显示鼠标指针请别开启该功能）")
waydroidRepairUnshowMouseOn = QtGui.QAction("开启")
waydroidRepairUnshowMouseOff = QtGui.QAction("关闭")
...
waydroidSetting = QtGui.QAction("Waydroid 设置")
waydroidFileManager = QtGui.QAction("Waydroid 文件")
waydroidBrowser = QtGui.QAction("Waydroid 浏览器")
...
waydroidContainerStart = QtGui.QAction("开启")
waydroidContainerStop = QtGui.QAction("关闭")
waydroidContainerRestart = QtGui.QAction("重新启动")
waydroidContainerFreeze = QtGui.QAction("Freeze")
waydroidContainerUnfreeze = QtGui.QAction("Unfreeze")
...
widevineInstall.triggered.connect(lambda: threading.Thread(target=RunBash, args=[f"python3 '{programPath}/Runner_tools/Widevine_Installer/widevine-installer.py'"]).start())
...
# 帮助 栏
helpAction = QtGui.QAction("程序帮助")
uploadBugAction = QtGui.QAction("问题反馈")
updateProgram = QtGui.QAction("更新程序")
programWebsite = QtGui.QAction("程序地址")
aboutThisProgramAction = QtGui.QAction("关于本程序(&A)")
